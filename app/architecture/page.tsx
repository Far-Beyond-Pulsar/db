...
const LAYERS = [
  ...
  },
  {
    ...
    name: "Spatial",
    tag: "CPU · Layer 1",
    desc: "Six dedicated f32 columns for AABB min/max per axis. Queries scan column arrays directly — no per-entity iteration, no allocation. Scalar reference must match SIMD bit-for-bit.",
  ...
  },
  {
    ...
    name: "Streaming",
    tag: "CPU · Layer 1",
    desc: "Concentric classification into Outer / Margin / Inner domains with hysteresis bands that damp boundary jitter. Multiple observer AABBs and persistent pins coexist on the same grid.",
  ...
  },
  {
    ...
    desc: "Zero-sized witnesses gate every phase: SimulateWitness to write, HarvestPhase to read back, RetiredPhase to compact. Invalid transitions are unrepresentable.",
    items: ["SimulateWitness", "HarvestPhase", "RetiredPhase"],
  },
  {
    icon: Radio,
    name: "Replication",
...
const FLOW_CODE = `// The driver in gpu::phase produces and consumes witnesses in order:
// acquire → simulate → harvest → boundary → repeat.
//
//   SimulateWitness  (write to cells, push to GPU)
//         ↓
//   HarvestPhase     (snapshot liveness, per-view queries)
//         ↓
//   RetiredPhase     (retire, compact, execute transitions)
//         ↓
//   back to SimulateWitness`;
      ...
      <div className="max-w-6xl mx-auto px-5 pb-24">
        ...
        >
          ...
          <p className="text-white/40 max-w-2xl text-sm leading-relaxed">
            SceneDB is layered. The bottom is a paged storage engine; spatial, streaming, and GPU
            layers sit above it; a compile-time phase machine enforces ordering; and the
            replication layer rides on top of the frame boundary.
          </p>
        </motion.div>
        ...
        >
          <div className="flex flex-col md:flex-row items-stretch gap-2">
            {[
              { icon: CpuIcon, name: "Acquire" },
              { icon: CpuIcon, name: "Simulate" },
